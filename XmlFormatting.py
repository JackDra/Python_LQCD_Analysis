...
def GetInfoFromFilelist(filedict):
    dictout = OrderedDict()
    dictout['nconfig'] = len(list(filedict.keys()))
    lenths = []
    for ikey,cfglist in filedict.items():
...
def unxmlcut(icut):
    if '-' in icut:
        return list(map(int,icut.replace('cut','').split('-')))
    else:
...
def FormatToDictAvgStd(String):
    return dict(list(zip(['Avg','Std'],FormatToAvgStd(String))))
...
def FormatToDictAvgStdChi(String):
    return dict(list(zip(['Avg','Std','Chi'],FormatToAvgStdChi(String))))
...
def unxmlfitr_int(thefitr):
    return list(map(int,unxmlfitr(thefitr)))
...
class KeyForamtting(object):
    ...
    def FmtTflow(self,tflow,phys=False):
        is_bytes = isinstance(tflow,bytes)
        if is_bytes: tflow = str(tflow)
        if isinstance(tflow,str):
            ...
            tflow_out = tflow.replace(tflow_numb,tflow_numb_new)
            if is_bytes: tflow_out = bytes(tflow_out,'utf8')
            return tflow_out
        else:
            if phys:
    ...
    def FormatSeriesKeys(self,this_series,physical=True):
        ...
        for icol,coldata in this_series[this_list].items():
            icol = str(icol)
