...
class VariationalMethod(object):
    ...
    def PlotEffMass_Boot(self,**kwargs):
    ...
    def PlotEffMass_VarMeth_Boot(self,**kwargs):
    ...
    def PlotEffMass(self,**kwargs):
        if hasattr(self,'booted_mcorr')  and self.booted_mcorr is not None:
            return self.PlotEffMass_Boot(**kwargs)
    ...
    def PlotEffMass_VarMeth(self,**kwargs):
        if hasattr(self,'booted_proj_corr')  and self.booted_proj_corr is not None:
            return self.PlotEffMass_VarMeth_Boot(**kwargs)
        if not hasattr(self,'eff_mass'):
            print('effective mass not computed, computing now')
...
if __name__ == '__main__':
    ...
    this_info = pa.Series()
    this_info['save_file'] = '~/TestGraphs/TestVar.pdf'
    this_info['title'] = 'Var Meth'
    this_info['x_label'] = 't'
    this_info['y_label'] = 'EffMass'
    # data_plot = jpl.Plotting(plot_info=this_info)
    plot_data = test_class.PlotEffMass()
    plot_data_varmeth = test_class.PlotEffMass_VarMeth()
    plot_data['y_data'] = plot_data['y_data'].reset_index()
    plot_data['y_data'][plot_data['y_data']['ism'] == '0']
    plot_data['y_data'][plot_data['y_data']['ism'] == '0']
    plot_data_varmeth['y_data'].reset_index()
# C_ij = sum_a Z^a_ij * exp(-E^a t)
# C_a = Z^a exp(-E^a t)
# [config, ism, jsm, t]
# Z^a_ij = Z^a_i*Z^a_j
